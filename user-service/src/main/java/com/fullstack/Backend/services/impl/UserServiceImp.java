...
import com.fullstack.Backend.responses.users.*;
...
public class UserServiceImp implements UserService, UserDetailsService {
    ...
    public CompletableFuture<ResponseEntity<Object>> authenticateUser(LoginDTO loginRequest, Authentication authentication) {
        ...
        return CompletableFuture.completedFuture(ResponseEntity
                ...
                .body(new JwtResponse(userDetails
                        .getUser()
                        .getId(), userDetails.getUsername(), userDetails.getEmail(), roles, userDetails
                        .getUser()
                        .getBadgeId(), userDetails
                        .getUser()
                        .getFirstName(), userDetails
                        .getUser()
                        .getLastName(), userDetails
                        .getUser()
                        .getPhoneNumber(), userDetails
                        .getUser()
                        .getProject())));
    }
    ...
    public CompletableFuture<ResponseEntity<Object>> showUsersWithPaging(int pageIndex, int pageSize, String sortBy, String sortDir, FilterUserDTO dto) throws ExecutionException, InterruptedException {
        ...
        List<String> projectList = usersList
                .stream()
                .map(UserDTO::getProject)
                .distinct()
                .toList();
    ...
    public CompletableFuture<ResponseEntity<Object>> sendResetPasswordEmail(String siteURL, String userEmail) throws ExecutionException, InterruptedException, MessagingException {
        ...
        if(user == null) return CompletableFuture.completedFuture(ResponseEntity
                .badRequest()
                .body(new MessageResponse("Email is not valid!")));

        PasswordResetToken existingToken = findUserFromResetPasswordToken(user);
        if(existingToken != null) {
            String newToken = generateResetPasswordToken(
                    existingToken.getToken()); /* Change old token to new token and return it */
    ...
    public CompletableFuture<ResponseEntity<Object>> saveResetPassword(ResetPasswordDTO dto) throws ExecutionException, InterruptedException, MessagingException {
        ...
        if(user.isEmpty()) return CompletableFuture.completedFuture(ResponseEntity
                .badRequest()
                .body(new MessageResponse("User is not existent")));

        boolean isOldPasswordSimilarToNewOne = BCrypt.checkpw(dto.getOldPassword(), user
                .get()
                .getPassword());
    ...
    public CompletableFuture<ResponseEntity<Object>> saveForgotPassword(ForgotPasswordDTO dto) throws ExecutionException, InterruptedException, MessagingException {
        ...
        if(token == null) return CompletableFuture.completedFuture(ResponseEntity
                .badRequest()
                .body(new MessageResponse("Token is not existent")));

        if(!Objects.equals(dto.getNewPassword(), dto.getConfirmPassword())) return CompletableFuture.completedFuture(
                ResponseEntity
                        .badRequest()
    ...
    public CompletableFuture<ResponseEntity<Object>> verifyPasswordToken(String token) {
        ...
        boolean isTokenExpired = passToken
                .getExpiryDate()
                .before(cal.getTime());
        if(isTokenExpired) {
    ...
    public CompletableFuture<ResponseEntity<Object>> getSuggestKeywordUsers(int fieldColumn, String keyword, FilterUserDTO filter) throws InterruptedException, ExecutionException {
        if(keyword
                .trim()
                .isBlank()) return CompletableFuture.completedFuture(ResponseEntity
                .status(NOT_FOUND)
    ...
    private Set<String> selectColumnForKeywordSuggestion(List<UserDTO> userList, String keyword, int fieldColumn) {
        ...
        switch (fieldColumn) { /*Fetch only one column*/
            case USER_BADGE_ID_COLUMN -> mappedDeviceList = userList
                    .stream()
                    .map(UserDTO::getBadgeId);
            case USER_NAME_COLUMN -> mappedDeviceList = userList
                    .stream()
                    .map(UserDTO::getUserName);
            case USER_FIRST_NAME_COLUMN -> mappedDeviceList = userList
                    .stream()
                    .map(UserDTO::getFirstName);
            case USER_LAST_NAME_COLUMN -> mappedDeviceList = userList
                    .stream()
                    .map(UserDTO::getLastName);
            case USER_EMAIL_COLUMN -> mappedDeviceList = userList
                    .stream()
                    .map(UserDTO::getEmail);
            case USER_PHONE_NUMBER_COLUMN -> mappedDeviceList = userList
                    .stream()
    ...
    private void formatFilter(FilterUserDTO dto) {
        if(dto.getBadgeId() != null) dto.setBadgeId(dto
                .getBadgeId()
                .trim()
                .toLowerCase());

        if(dto.getUserName() != null) dto.setUserName(dto
                .getUserName()
                .trim()
                .toLowerCase());

        if(dto.getFirstName() != null) dto.setFirstName(dto
                .getFirstName()
                .trim()
                .toLowerCase());

        if(dto.getLastName() != null) dto.setLastName(dto
                .getLastName()
                .trim()
                .toLowerCase());

        if(dto.getEmail() != null) dto.setEmail(dto
                .getEmail()
                .trim()
                .toLowerCase());

        if(dto.getPhoneNumber() != null) dto.setPhoneNumber(dto
                .getPhoneNumber()
                .trim()
                .toLowerCase());

        if(dto.getProject() != null) dto.setProject(dto
                .getProject()
                .trim()
                .toLowerCase());
    ...
    private List<User> fetchFilteredUsers(FilterUserDTO dto, List<User> users) {
        if(dto.getBadgeId() != null) users = users
                .stream()
                .filter(user -> user
                        .getBadgeId()
                        .equalsIgnoreCase(dto.getBadgeId()))
                .collect(Collectors.toList());
        if(dto.getUserName() != null) users = users
                .stream()
                .filter(user -> user
                        .getUserName()
                        .equalsIgnoreCase(dto.getUserName()))
                .collect(Collectors.toList());
        if(dto.getFirstName() != null) users = users
                .stream()
                .filter(user -> user
                        .getFirstName()
                        .equalsIgnoreCase(dto.getFirstName()))
                .collect(Collectors.toList());
        if(dto.getLastName() != null) users = users
                .stream()
                .filter(user -> user
                        .getLastName()
                        .equalsIgnoreCase(dto.getLastName()))
                .collect(Collectors.toList());
        if(dto.getEmail() != null) users = users
                .stream()
                .filter(user -> user
                        .getEmail()
                        .equalsIgnoreCase(dto.getEmail()))
                .collect(Collectors.toList());
        if(dto.getPhoneNumber() != null) users = users
                .stream()
                .filter(user -> user
                        .getPhoneNumber()
                        .equalsIgnoreCase(dto.getPhoneNumber()))
                .collect(Collectors.toList());
        if(dto.getProject() != null) users = users
                .stream()
                .filter(user -> user
                        .getProject()
